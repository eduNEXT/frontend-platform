...
export function generateAPIConfigUrl() {
  var _getConfig = getConfig(),
      ...
      MFE_CONFIG_API_PATH = _getConfig.MFE_CONFIG_API_PATH,
      MFE_CONFIG_API_PORT = _getConfig.MFE_CONFIG_API_PORT,
      MFE_CONFIG_API_REGEX = _getConfig.MFE_CONFIG_API_REGEX;

  if (!MFE_CONFIG_API_PATH || !MFE_CONFIG_API_REGEX) {
    return null;
  }

  var regex = new RegExp(MFE_CONFIG_API_REGEX, 'g');
  var host = (window.location.hostname.match(regex) || []).join('.');
  ...
  var queryParams = APP_ID ? "?".concat(params.toString()) : '';
  var url = new URL("".concat(window.location.protocol, "//").concat(host, "/").concat(MFE_CONFIG_API_PATH).concat(queryParams));
